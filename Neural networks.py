#Autor programu:Aleh Iotchanka 
#Perceptron dwuwarstwowy
# funkcja : J(x) = sin(x*pierwiastek(p[1]+1))+cos(x*pierwiastek(p[2]+1))
import math as m
import random
...
def x_gen(down_limit, up_limit):        #generacja x
    return random.uniform(down_limit, up_limit)

def function(x):   #funkcja wejściowa
    return m.sin(x * (m.sqrt(p1 + 1))) + m.cos(x * (m.sqrt(p2 + 1)))

def test_generate(down_limit, up_limit, size):    #generacja danych testowych
    x_set, y_set = [], []
    for _ in range(size):
...
class Linear_neuron(Neuron):   #klasa neuronu liniowego
    def __init__(self, number):
...
class Perceptron:   #klasa perceptron dwuwarstwowy
    ...
    def verify(self, test_x, test_y):
        ...
        print("najmniejszy błąd: ", min(e))
        print("największy błąd: ", max(e))
        print("średni błąd aproksymacji: ", sum(e)/len(e))
        print("odchylenie standardowe błędu:", statistics.stdev(e))
